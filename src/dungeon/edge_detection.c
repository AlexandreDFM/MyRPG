    ...
    for (int i = 0; tab[i]; i++) {
        for (int o = 0; tab[i][o] != '\0'; o++) {
            ...
            if (tab[i][o] == room) {
                ...
                insert_into(&pixels, ind, i, o, ptr, r, line_len, 144);
            } else {
                int r = (rand() % 100) >= 5 ? 0 : 1;
                insert_into(&pixels, ind, i, o, ptr, 3 + r, line_len, 144);
            }
        }
    }
    sfImage *texture = sfImage_createFromPixels(24 * line_len, 24 * line_len, pixels);
    return texture;
}
...
void insert_into(sfUint8 **pixels, int index, int start_x, int start_y, const sfUint8 *ptr, int y, int line_len, int ref_len)
{
    for (int x = 0; x < 24; x += 1) {
        for (int i = 0; i < 24 * 4; i++) {
            (*pixels)[(i + (24 * 4) * start_y) + (x + 24 * start_x) * (24 * line_len * 4)] = ptr[i + (x + 24 * index) * (ref_len * 4) + 24 * 4 * y];
        }
    }
}
...
int search(int array[], int x, int started) {
    ...
    if (started) {
        if (x == 159 || x == 63) {
            ...
            return 20;
        } else if (x == 188 || x == 185 || x == 153 || x == 156 || x == 57 || x == 60 || x == 25 || x == 28 || x == 152 || x == 56) {
            return 10;
        } else if (x == 232 || x == 105) {
            return 6;
        } else if (x == 71 || x == 98 || x == 99 || x == 2 || x == 70 || x == 194 || x == 67 || x == 226 || x == 195) {
            return 12;
        } else if (x == 234) {
            return 33;
        } else if (x == 59) {
