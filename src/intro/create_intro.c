...
intro_a *create_intro(wininf *inf)
{
    ...
    for (int i = 1, j = 5; i < 4; i++, j++)
        intro->pkmintro[i] = c_ele(inf->atlases.atlas, "./csv/intro.csv", j);
    ...
    intro->i_music1 =
    my_music_from_file("music/01_pkm_red_rescue_team_theme.ogg");
    intro->i_music2 = my_music_from_file("music/02_title_screen.ogg");
    return intro;
}
