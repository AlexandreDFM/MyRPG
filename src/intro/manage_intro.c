...
void manage_intro(wininf *inf)
{
    ...
    if (inf->intro->valid == 0 && inf->c_scene == INTRO && inf->intro->plan
    != PLAN6) {
        if (inf->event.type == sfEvtKeyPressed || (inf->event.mouseButton.type
        == sfEvtMouseButtonPressed && inf->event.mouseButton.button
        != sfMouseRight)) {
    ...
    (inf->event.mouseButton.type == sfEvtMouseButtonReleased &&
    inf->event.mouseButton.button != sfMouseRight))) {
        inf->intro->valid += 1;
    }
    if (inf->intro->valid != 0 && inf->c_scene == INTRO &&
    inf->intro->plan == PLAN6) {
        if (inf->intro->valid >= 2 && (inf->event.type == sfEvtKeyPressed ||
        (inf->event.mouseButton.type == sfEvtMouseButtonPressed &&
        inf->event.mouseButton.button != sfMouseRight))) {
